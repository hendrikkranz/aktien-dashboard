...
df = load_portfolio("data/dauergewinner.csv")

# Einstand inklusive Kaufspesen
df["Spesen EUR"] = df["Spesen EUR"].fillna(0)
df["Einstand EUR"] = (
    df["Kaufwert EUR"] + df["Spesen EUR"]
)

# Live-Depotwert
df["Live-Wert EUR"] = (
    df["Stück"] * df["Live-Kurs EUR"]
)

# Falls kein Live-Kurs verfügbar ist, CSV-Wert verwenden
df["Berechneter Wert EUR"] = (
    df["Live-Wert EUR"].fillna(
